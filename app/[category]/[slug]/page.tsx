...
import { ARTICLE_QUERY, ARTICLE_SLUGS_QUERY, RELATED_ARTICLES_QUERY } from "@/sanity/queries";
...
export async function generateStaticParams() {
  const articles = await staticParamsClient.fetch<{ category: string | null; slug: string }[]>(
    ARTICLE_SLUGS_QUERY
  );
  return articles
...
export default async function ArticlePage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10">
      ...
      {article.faq && article.faq.length > 0 && <JsonLd data={buildFaqJsonLd(article.faq)} />}
      ...
      <header className="mt-4 border-b border-line pb-6">
        ...
        <p className="mt-3 font-body text-lg text-ink-soft">{article.metaDescription}</p>
        <div className="mt-5">
          <AuthorByline
      ...
      <p className="mt-10 font-ui text-xs text-ink-soft">
        Prices and availability change often; verify the current price before you buy. Last
